...
let publicKeyPem = sessionStorage.getItem("publicKey"); // <<< ADDED THIS LINE (your own public key)
...
let ownPublicKeyObject = null; // To store your own imported public key object
...
    // Check if user is logged in and has keys
    if (!currentUser || !privateKeyPem || !publicKeyPem) { // <<< CHECK FOR publicKeyPem TOO <<<
...
    try {
        // Import your own public key once at startup
...
        // Consider disabling message input or sending if this fails critically
    }
...
// --- Utility Functions for UI and Cryptography (NO CHANGES HERE) ---
...
// --- Friends List Management (NO CHANGES HERE) ---
...
// --- User Interaction Functions (NO CHANGES HERE, except the fix inside sendMessage) ---
...
    // Ensure your own public key is loaded
    if (!ownPublicKeyObject) { // <<< USE THE PRE-IMPORTED OBJECT <<<
...
        // 2. Encrypt message for self using YOUR OWN public key (already loaded at startup)
        const encryptedMessageForSelf = await encryptMessage(message, ownPublicKeyObject); // <<< FIXED LINE <<<
